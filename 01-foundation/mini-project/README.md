...
- [ ] Double-entry invariant: <mark>her transfer için 2 journal_line, toplam debit = toplam credit</mark>
...
**Ne yapacaksın:** Domain'i PostgreSQL'e bağlayan adapter katmanını yazacaksın: JPA entity, mapper ve port implementation'ı. **Neden:** Hexagonal'da <mark>domain veritabanını bilmez</mark>; araya bu adapter girer ve dönüşümü üstlenir.
...
**Ne yapacaksın:** Topic 1.5'te taslak olarak yazdığın controller'ları gerçek use case'lere bağlayacaksın. **Neden:** <mark>Controller sadece çevirmendir</mark> — HTTP'yi domain diline çevirir, iş kuralı barındırmaz.
...
**Ne yapacaksın:** Idempotency key'leri veritabanında saklayacaksın. **Neden:** Client aynı transferi iki kez gönderirse (retry, timeout) <mark>para iki kez gitmemeli</mark> — key'i kalıcı tutmadan bunu garanti edemezsin.
